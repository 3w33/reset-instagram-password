...
class DESIGN():
    ...
    xrblue = f"\n{blueplus} Instagram Password Reset {BLUE}/ {WHITE}Instagram{BLUE}: {WHITE}@bfm2{BLUE}"
class bfm2():
    def __init__(self):
        print(DESIGN.xrblue)
        ...
        self.target = input()
        if self.target[0]=="@":
            print(f"\n{DESIGN.redminus} Enter Username Without {DESIGN.RED}@")
            self.inex()
        if "@" in self.target:
            usem = "user_email"
        else:
    ...
    def send_password_reset(self):
        ...
        if "obfuscated_email" in req.text: 
            ...
            print(f'\n{DESIGN.redminus} There Is No One With The Username @{self.target} .')
            self.inex()
        else:
